...
namespace Microsoft.MixedReality.OpenXR.Samples
{
    public class Hand
    {
        ...
        private readonly GameObject[][] handFingerGameObjects = new GameObject[HandFingers.Length][];
        ...
        private readonly GameObject[] handJointGameObjects = new GameObject[HandJoints.Length];
        ...
        public void UpdateHandJoints(HandJointLocation[] locations)
        {
            ...
            {
                int handJointInt = (int)handJoint;
                if (handJointGameObjects[handJointInt] == null)
                {
                    ...
                    handJointGameObjects[handJointInt] = jointObject;
                }

                GameObject handJointGameObject = handJointGameObjects[handJointInt];
                HandJointLocation handJointLocation = locations[handJointInt];
        ...
        public void UpdateHandJoints(UnityEngine.XR.Hand hand)
        {
            ...
            {
                int fingerInt = (int)finger;
                if (hand.TryGetFingerBones(finger, fingerBones))
                {
                    if (handFingerGameObjects[fingerInt] == null)
                    {
                        ...
                        handFingerGameObjects[fingerInt] = jointArray;
                    }

                    GameObject[] fingerJointGameObjects = handFingerGameObjects[fingerInt];
                    ...
                    for (int i = 0; i < fingerBones.Count; i++)
                    {
                        ...
                        if (bone.TryGetParentBone(out Bone parentBone) && parentBone.TryGetPosition(out Vector3 parentPosition))
                        {
                            Debug.DrawLine(parentPosition, position, LookupColor(finger, i));
                        }
        ...
        {
            jointObject.GetComponent<Renderer>().material.color = LookupColor(finger, index);
        }

        private static Color LookupColor(HandFinger? finger, int? index)
        {
